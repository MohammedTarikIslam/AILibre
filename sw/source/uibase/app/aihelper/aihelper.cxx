...
void set_selected(const OUString& result)
{
    Reference<uno::XComponentContext> xContext = comphelper::getProcessComponentContext();
    auto xModel = getModel(xContext);
    uno::Reference<frame::XController> xCtrl = xModel->getCurrentController();
...
OUString query_llama(httplib::Client& client, const std::string& prompt)
{
    ...
    nlohmann::json payload = { { "prompt", prompt },   { "stop", { "###" } }, { "n_predict", 128 },
...
OUString chunker(const std::string& text, httplib::Client& client, const std::string command)
{
    ...
    else
    {
        ...
        for (int i = 0; i < subchunks.size(); ++i)
        {
            ...
            OUString res = query_llama(client, sub_prompt);
            const char* res_text = res.toUtf8().getStr();
            results.push_back("[Part " + std::to_string(i) + "]" + res_text);
        }
        ...
        std::string final_prompt = "### Instruction:\n"
                                   "Combine the set of small summaries which are parts of a large "
                                   "text into a single cohesive summary.\n\n"
                                   "### Parts:\n"
                                   + merged + "\n\n### Response:\n";
...
OUString ProcessAICommand()
{
    //const OUString& rSelectedText, const OUString& rCommandType
    httplib::Client client("127.0.0.1", 8080);
    std::string text = std::string(get_selected().toUtf8());
    std::string command = "summarise";
    //std::string(rCommandType.toUtf8());

    auto result = chunker(text, client, command);
